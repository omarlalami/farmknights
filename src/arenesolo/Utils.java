...
public class Utils {
	...
	public Action deplacement(Point actuel,Point dest) {
    	...
    	//une des deux coordonn�es de la destination(x ou y) sera diff�rente du X ou Y de la position courante
    	
    	//ici on test si c'est les X qui sont diff�rents 
    	if (dest.x!=myx) {
    		// si le x de la destination est sup�rieur on va � droite
    		if (dest.x>myx) return Action.DROITE;
    		//sinon on va � gauche
    		else return Action.GAUCHE;
    	}
    	//ici on test si c'est les Y qui sont diff�rents 
    	if (dest.y!=myy) {
    		// si le y de la destination est sup�rieur on va en bas
